...
class VPRModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        images = torch.concat([places_1, places_2], dim=0).view(
            (places_1.size(0) + places_2.size(0)) * N, ch, h, w
        )
        ...
        loss_1 = self.loss_function(
            descriptors[: places_1.size(0) * N], labels[: places_1.size(0) * N]
        )
        loss_2 = self.loss_function(
            descriptors[places_2.size(0) * N : images.size(0)],
            labels[places_2.size(0) * N :],
        )

        embs_paired = descriptors[pairs_3.squeeze(0)]
        loss_3 = self.gcl_loss(
            embs_paired[:, 0], embs_paired[:, 1], scores_3.squeeze(0)
        )
