...
def load_city_df(base_path):
    ...
    for city in (Path(base_path)).iterdir():
        ...
        db = db.join(
            pd.read_csv(city / "database" / "raw.csv")[["pano", "key"]].set_index(
                "key"
            ),
            on="key",
        )
...
def compute_cluster_descriptors(
    city_df, model, descriptor_size=8192 + 256, batch_size=64
):
    ...
    class MSLSDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            ...
            path = (
                Path(BASE_PATH)
                / self.city_path
                / ("query" if row["query"] else "database")
                / "images"
                / f'{row["key"]}.jpg'
            )
            try:
    ...
    for city, df in tqdm.tqdm(city_df.items(), desc="Computing cluster descriptors"):
        ...
        cluster_descriptors = torch.zeros(
            (df.unique_cluster.max() + 1, descriptor_size)
        ).cuda()
    ...
    for i in tqdm.tqdm(range(num_batches)):
        ...
        while batch_idx < batch_size:
            ...
            num_clusters = np.array(
                [d.shape[0] for c, d in cluster_descriptors_dict.items()]
            )

            city = np.random.choice(
                cities_to_sample, p=num_clusters / num_clusters.sum()
            )
            ...
            other_places = np.random.choice(
                np.arange(df.unique_cluster.max() + 1),
                size=sampled_similar_places,
                p=distances,
                replace=False,
            )
            ...
            utms = (
                squareform(pdist(df[["easting", "northing"]].values))
            ...
            while batch_idx < batch_size:
                ...
                for c in networkx.find_cliques(networkx.Graph(utms)):
                    if len(c) >= num_images_per_place:
                        clique = np.random.choice(
                            c, num_images_per_place, replace=False
                        )
                        break
                else:
                    break
                ...
                images[i, batch_idx] = np.char.add(
                    np.char.add(
                        np.where(
                            rows["query"].values,
                            f"{city}/query/images/",
                            f"{city}/database/images/",
                        ).astype("<U100"),
                        rows["key"].values.astype("<U100"),
                    ),
                    ".jpg",
                )
                batch_idx += 1
...
class CliqueMapillaryDataset(Dataset):
    ...
    def create_dataset(
        ...
        cluster_descriptors_path = (
            Path(__file__).parent.parent
        ) / "cluster_descriptors.npy"
        ...
        if model is not None:
            ...
            cluster_descriptors_dict = np.load(
                cluster_descriptors_path, allow_pickle=True
            ).item()
        else:
