...
class GSVCitiesDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        self.train_dataset_2 = CliqueMapillaryDataset(
            self.train_transform, **self.clique_mapillary_args
        )
        if stage == "fit":
    ...
    def train_dataloader(self):
        self.reload()
        return {
            "GSVCities": DataLoader(
                dataset=self.train_dataset_1, **self.train_loader_config
            ),
            "MSLS": DataLoader(
                dataset=self.train_dataset_2, **self.train_loader_config_2
            ),
            "aachen": DataLoader(
                self.train_dataset_3, batch_size=1, shuffle=False, num_workers=4
            ),
        }
